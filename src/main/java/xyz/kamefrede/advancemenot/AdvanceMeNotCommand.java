...
public class AdvanceMeNotCommand {
	public static final SimpleCommandExceptionType NO_DISALLOWED_DIMENSIONS = new SimpleCommandExceptionType(new TranslationTextComponent("advancemenot.empty_disallowed_list"));
	public static final DynamicCommandExceptionType DIMENSION_NOT_PRESENT_IN_DISALLOWED_LIST = new DynamicCommandExceptionType((dimension) -> new TranslationTextComponent("advancemenot.dimension_not_present", dimension));

	public static void register(CommandDispatcher<CommandSource> dispatcher) {
		LiteralArgumentBuilder<CommandSource> commandBuilder = Commands.literal("advancemenot")
				...
				.then(Commands.literal("add")
						.then(Commands.argument("dimension", DimensionArgument.getDimension())
								.suggests((context, builder) -> {
									...
									context.getSource().sendFeedback(new TranslationTextComponent("advancemenot.added_dimension", dimension.getLocation().toString()), true);
						...
						.then(Commands.argument("dimension", DimensionArgument.getDimension()).suggests((context, builder) -> {
							...
							return ISuggestionProvider.func_212476_a(Config.disallowedDimensions.stream().map(RegistryKey::getLocation), builder);
								...
								.executes(context -> {
									...
									Config.updateConfig();
									context.getSource().sendFeedback(new TranslationTextComponent("advancemenot.removed_dimension", dimension.getLocation().toString()), true);
									return Command.SINGLE_SUCCESS;
								})))
				.then(Commands.literal("get")
						.executes(context -> {
							if (Config.disallowedDimensions.isEmpty()) {
								...
								context.getSource().sendFeedback(new TranslationTextComponent("advancemenot.disallow_dimensions_print", sb), true);
							}
							return Command.SINGLE_SUCCESS;
						}));
